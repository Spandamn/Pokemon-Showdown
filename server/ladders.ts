...
class Challenge {
	readonly from: User | User[];
	readonly to: User | User[];
	readonly formatid: string;
	readonly ready: BattleReady;
	constructor(ready: BattleReady, to: User | User[]) {
		if (ready.challengeType === 'multi') {
			this.from = Object.assign([], ready.players);
			this.to = Object.assign([], to);
		} else {
			this.from = ready.players;
			this.to = to;
		}
...
class Ladder extends LadderStore {
	...
	async prepBattle(connection: Connection, challengeType: ChallengeType, team: string | null = null, isRated = false, teammateCon: Connection | null = null, teammateTeam: string | null = null) {
		...
		for (let i in matches) {
			...
			while (match) {
				...
				if (nickname) {
					const curUser = (i === '1' ? connection : teammateCon);
		...
		if (isRated && !Ladders.disabled && challengeType !== 'multi') {
			const uid = user.id;
			[valResult1, rating] = await Promise.all([
	...
	static getChallenging(user: User) {
		...
		if (userChalls) {
			for (const chall of userChalls) {
				if (chall.from === user) return chall;
			}
		}
		return null;
	}
	...
	static clearChallenges(username: string) {
		...
		if (userChalls) {
			for (const chall of userChalls.slice()) {
				let otherUserid;
				if (chall.from === user) {
					if (Array.isArray(chall.to)) {
						otherUserid = chall.to[0];
					} else {
						otherUserid = chall.to.id
					}
				} else {
					if (Array.isArray(chall.from)) {
						otherUserid = chall.from[0];
					} else {
						otherUserid = chall.from.id
					}
				}
	...
	async makeChallenge(connection: Connection, targetUser: User, teammate: User) {
		...
		if (teammate.settings.blockChallenges && !user.can('bypassblocks', teammate)) {
		...
		if (Dex.getFormat(chall.formatid).gameType === "multi" && teammate === connection.user) {
	...
	static async acceptChallenge(connection: Connection, targetUser: User, asTeammate: boolean?) {
		const chall = Ladder.getChallenging(targetUser);
		const isMultiBattle = false;
		if (Dex.getFormat(chall.formatid).gameType === 'multi' && asTeammate && chall.from.length < 2) {
			chall.from.push(connection.user);
			targetUser.connections[0].popup(`${connection.user.name} has accepted you team invite!`);
		}
		if (!chall || chall.to !== connection.user) {
	...
	static match(ready1: BattleReady, ready2: BattleReady) {
		...
		if (Array.isArray(ready1.players) && Array.isArray(ready2.players) && ready1.challengeType === "multi" && ready2.challengeType === "multi") {
			...
			for (let player in team1) {
				if (!team1[player]) {
					team1[player ^ 1].popup(`Sorry, your teammate ${team1[player].id} went offline before your battle could start.`);
			...
			for (let player in team2) {
				if (!team2[player]) {
					team2[player ^ 1].popup(`Sorry, your teammate ${team2[player].id} went offline before your battle could start.`);
		...
		const user1 = ready1.players;
		const user2 = ready2.players;
		if (!user1) {
